...
namespace CGL {
     ...
     */
    PathtracerRenderer::PathtracerRenderer(size_t ns_aa,
     ...
     */
    void PathtracerRenderer::set_camera(Camera *camera) {
     ...
     */
    void PathtracerRenderer::set_frame_size(size_t width, size_t height) {
     ...
     */
    void PathtracerRenderer::start_visualizing() {
        if (state != READY) {
            return;
        }
        state = VISUALIZE;
     ...
     */
    void PathtracerRenderer::start_raytracing() {
        if (state != READY) return;
        ...
        for (int i = 0; i < numWorkerThreads; i++) {
            workerThreads[i] = new std::thread(&PathtracerRenderer::worker_thread, this);
        }
    }

    void PathtracerRenderer::render_to_file(string filename, size_t x, size_t y, size_t dx, size_t dy) {
        if (x == -1) {
    ...
    void PathtracerRenderer::key_press(int key) {
        ...
        case 'l': case 'L':
            ...
            break;
        case 'q': case 'Q':
            if (pt->isAccumBounces > 0)
                pt->isAccumBounces = 0;
            else
                pt->isAccumBounces = 1;
            if (pt->isAccumBounces)
                fprintf(stdout, "[PathTracer] Accumulate bounces changed to true.\n");
            else
                fprintf(stdout, "[PathTracer] Accumulate bounces changed to false.\n");
            break;
        case 'G': case 'g':
            pt->use_roulette_stopping = !pt->use_roulette_stopping;
            if (pt->use_roulette_stopping)
                fprintf(stdout, "[PathTracer] Using russian roulette is set to true.\n");
            else
                fprintf(stdout, "[PathTracer] Using russian roulette is set to false.\n");
            break;
        case ';':
     ...
     * in a worker thread.
     */
    void PathtracerRenderer::raytrace_tile(int tile_x, int tile_y,
    ...
    void PathtracerRenderer::raytrace_cell(ImageBuffer &buffer) {
    ...
    void PathtracerRenderer::autofocus(Vector2D loc) {
        pt->autofocus(loc);
    }

    void PathtracerRenderer::worker_thread() {
    ...
    void PathtracerRenderer::save_image(string filename, ImageBuffer *buffer) {
    ...
    void PathtracerRenderer::save_sampling_rate_image(string filename) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                float samplingRate = pt->sampleCountBuffer[y * w + x] * minv;
